...
def _engine_readme(role: str, recipe: dict, n: int) -> str:
...
from **{n}** logged interaction(s). The end state: the tool runs this role on your
...
def export_engine_bundle(project_dir: str | Path, role: str, *, base_model: str | None = None) -> TrainEngineResult:
    ...
    _write("README.md", _engine_readme(role, recipe, len(rows)))

    return TrainEngineResult(role=role, examples=len(rows), base_model=recipe["base_model"],
                             bundle_dir=str(out), files=files)
